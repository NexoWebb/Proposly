...
function EditorContent() {
  ...
  const persist = async () => {
    ...
    return supabase.from('proposals').insert({ user_id: uid, title, client_name: clientName, client_email: clientEmail, blocks, total_amount: computeTotal(blocks), status: 'draft', expires_at: expiresAt || null, public_token: crypto.randomUUID() }).select('id').single()
  }
